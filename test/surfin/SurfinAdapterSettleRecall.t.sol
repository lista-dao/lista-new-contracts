...
contract SurfinAdapterSettleRecall is Test {
  ...
  function _lockedMatured(address who, uint256 amount) internal {
    vm.warp(1_000_000);
    vm.prank(manager);
    locked.setCohort(1, 90, 114e15, block.timestamp + 1 days, block.timestamp + 1 days, block.timestamp + 91 days, true);
  ...
  function test_settleRecall_happy_covers_locked_sets_fee_book_and_buffer() public {
    ...
    assertEq(adapter.recallCycle(), block.timestamp / 1 weeks, "recall cycle marked (blocks same-week deploy)");
  }
