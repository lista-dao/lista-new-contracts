...
import { LendingRewardsDistributor } from "../src/LendingRewardsDistributor.sol";

contract LendingRewardsDistributorTest is Test {
  MockERC20 lista;
  LendingRewardsDistributor distributor;
  ...
  function setUp() public {
    ...
    LendingRewardsDistributor distributorImpl = new LendingRewardsDistributor();

    ERC1967Proxy proxy_ = new ERC1967Proxy(
      address(distributorImpl),
      abi.encodeWithSelector(LendingRewardsDistributor.initialize.selector, admin, manager, bot, pauser, address(lista))
    );

    distributor = LendingRewardsDistributor(address(proxy_));

    assertEq(address(lista), distributor.token());
  ...
  function test_emergencyWithdraw() public {
    ...
    vm.prank(manager);
    distributor.emergencyWithdraw(); // success
