  ...
  UUPSUpgradeable
{
  ...
  event RewardFunded(address indexed funder, address indexed token, uint256 amount);
   ...
   *      SurfinAdapter) can call; it pulls `amount` of `token` from itself.
   * @param token Address of the interest token (must be whitelisted)
   * @param amount Amount of interest to fund
   */
  function notifyReward(address token, uint256 amount) external onlyRole(FUNDER) whenNotPaused {
    require(tokens[token], "Token not supported");
    require(amount > 0, "Invalid amount");

    IERC20(token).safeTransferFrom(msg.sender, address(this), amount);

    emit RewardFunded(msg.sender, token, amount);
  }
