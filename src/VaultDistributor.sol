...
/**
 * @title Vault Distributor for Lista Lending
 * @author Lista DAO
 */
contract VaultDistributor is AccessControlEnumerableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  event Claimed(address indexed account, address indexed token, uint256 amount, uint256 lpAmount, uint256 totalAmount);
   ...
   * @param _lpAmount LP amount of the account at the snapshot block
  ...
  function claim(
    ...
    bytes32 leaf = keccak256(
      abi.encode(block.chainid, address(this), this.claim.selector, _account, _token, _lpAmount, _totalAmount)
    );
    ...
    emit Claimed(_account, _token, amount, _lpAmount, _totalAmount);
  }
